...
#define DIGTIME                                                         10000//Time of cooldown between digging.
#define VOMITTIME                                                       10000//Time of cooldown between vomitting.
//============================== [Dialogs] =====================================
...
public OnPlayerDeath(playerid,killerid,reason)
{
	...
	if(Team[killerid] == HUMAN && Team[playerid] == HUMAN)
	{
	    if(PInfo[playerid][Infected] == 1)
	    {
	        ...
	        if(PInfo[killerid][ShowingXP] == 0)
		    {
				static string[7];
				if(PInfo[killerid][Premium] == 2){PInfo[killerid][XP] += 14;PInfo[killerid][CurrentXP] = 14;}
				else {PInfo[killerid][XP] += 10;PInfo[killerid][CurrentXP] = 10;}
		        ...
		        SetTimerEx("ShowXP1",300,0,"d",killerid);
		        TextDrawShowForPlayer(killerid,GainXPTD[killerid]);
		        PlaySound(killerid,1083);
	...
	if(PInfo[playerid][Infected] == 1 && Team[playerid] == HUMAN)
	{
	    ...
	    PInfo[playerid][Deaths]++;
	    SetPlayerColor(playerid,purple);
		new id = PInfo[playerid][Lastbite];
		if(PInfo[id][ShowingXP] == 0)
  		{
			static string[7];
			if(PInfo[id][Premium] == 2){PInfo[id][XP] += 22;PInfo[id][CurrentXP] = 22;}
			else {PInfo[id][XP] += 12;PInfo[id][CurrentXP] = 12;}
			PInfo[id][InfectsRound]++;
			format(string,sizeof string,"+%i XP",PInfo[id][CurrentXP]);
			TextDrawSetString(GainXPTD[id],string);
			PInfo[id][ShowingXP] = 1;
		    SetTimerEx("ShowXP1",300,0,"d",id);
		    TextDrawShowForPlayer(id,GainXPTD[id]);
		    PlaySound(id,1083);
		}
		else
		{
	...
	if(Team[killerid] == ZOMBIE && Team[playerid] == HUMAN)
	{
	    ...
	    {
			static string[7];
			if(PInfo[killerid][Premium] == 2){PInfo[killerid][XP] += 22;PInfo[killerid][CurrentXP] = 22;}
			else {PInfo[killerid][XP] += 12;PInfo[killerid][CurrentXP] = 12;}
			PInfo[killerid][InfectsRound]++;
			format(string,sizeof string,"+%i XP",PInfo[killerid][CurrentXP]);
			TextDrawSetString(GainXPTD[killerid],string);
	        PInfo[killerid][ShowingXP] = 1;
	        SetTimerEx("ShowXP1",300,0,"d",killerid);
...
public OnPlayerConnect(playerid)
{
    ...
    PInfo[playerid][Sprinting] = 0;
    PInfo[playerid][Flamerounds] = 0;
    PInfo[playerid][MolotovMission] = 0;
    PInfo[playerid][CanDig] = DIGTIME;
    PInfo[playerid][GodDig] = 0;
    PInfo[playerid][SentDigMsg] = 1;
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if(PRESSED(KEY_CROUCH))
	{
			...
			else
			{
				...
				Random:
				{
					switch(rand)
					{
					    case 0:
					    {
					        if(PInfo[playerid][SPerk] == 18) goto Random;
					        else
		...
		if(Team[playerid] == ZOMBIE)
		{
		    ...
		    {
		        if(GetTickCount() - PInfo[playerid][CanDig] < DIGTIME) return SendClientMessage(playerid,red,"You are to tired to dig!");
		        new id = -1;
				...
				PInfo[playerid][CanDig] = GetTickCount();
				PInfo[playerid][SentDigMsg] = 0;
		    ...
		    if(PInfo[playerid][ZPerk] == 14)
		    {
		        ...
		        func:
		        {
					...
					if(Team[rand] == ZOMBIE)
					{
						if(count >= 1 || RoundEnded == 1) return SendClientMessage(playerid,white,"* "cred"Everyone is a zombie!");
						else
						{
						    count++;
							goto func;
	...
	if(HOLDING(KEY_SPRINT))
	{
	    ...
	    {
	        /*
            if(PInfo[playerid][Sprinting] == 0) ApplyAnimation(playerid, "PED" , "sprint_civi" , 2.0 , 1 , 1 , 1 , 0 , 0 , 1);
			if(PInfo[playerid][SprintTimer] == 0) SetTimerEx("SprintPlayer",1,false,"i",playerid),PInfo[playerid][SprintTimer] = 1;
			PInfo[playerid][Sprinting] = 1;*/
	    }
	}
	...
	if(PRESSED(KEY_HANDBRAKE))//Aim Key
	{
        if(!IsPlayerInAnyVehicle(playerid))
		{
            ...
            if(PInfo[playerid][ZPerk] == 8)
			{
                ...
                ApplyAnimation(playerid, "RIOT", "RIOT_shout", 3, 0, 0, 0, 1000, 1);
      		...
      		else if(PInfo[playerid][ZPerk] == 19)
			{
      		...
      		else
      		{
				...
				else {GetPlayerHealth(playerid,Health);SetPlayerHealth(playerid,Health+3);}
			 	MakeHealthEven(i,Health);
			    GetPlayerHealth(i,Health);
			    MakeHealthEven(i,Health);
				...
				if(PInfo[i][Infected] == 0) PInfo[i][Infected] = 1;
				ApplyAnimation(playerid,"WAYFARER","WF_FWD",1,0,0,0,0,300,1);
				if(Health <= 4.0)
				{
				    SetPlayerHealth(i,0.0);
				    GetPlayerPos(i,x,y,z);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == Humanperksdialog)
	{
	    ...
	    if(listitem == 12)
	    {
	        ShowPlayerDialog(playerid,Mechanicdialog,0,"More Ammo",""cwhite"This perk allows you to get more ammo, when you enter the checkpoint","Set","Cancel");
	    }
	    if(listitem == 13)
	    {
...
function UpdateStats()
{
	for(new i; i < MAX_PLAYERS;i++)
	{
		...
		if(Team[i] == ZOMBIE)
		{
			if((GetTickCount() - PInfo[i][CanDig]) >= DIGTIME && PInfo[i][SentDigMsg] == 0)
			    SendClientMessage(i,red,"You feel rested enough to dig again (digger ready)"),PInfo[i][SentDigMsg] = 1;
				...
				if(IsPlayerInRangeOfPoint(j,4.0,PInfo[i][Vomitx],PInfo[i][Vomity],PInfo[i][Vomitz]))
				{
				    ...
				    else
				    {
						...
						if(health <=4)
				        {
						    ...
						    {
								static string[7];
								if(PInfo[i][Premium] == 2){PInfo[i][XP] += 22;PInfo[i][CurrentXP] = 22;}
								else {PInfo[i][XP] += 12;PInfo[i][CurrentXP] = 12;}
...
public OnPlayerUseItem(playerid,ItemName[])
{
    ...
    if(!strcmp(ItemName,"Flashlight",true))
    {
        ...
        else
        {
        	...
        	SetPlayerAttachedObject(playerid, 0,354,5,0.10,0.03,-0.12,0.0,451.0,-190.0,0.00,0.92,1.00);
...
function SprintPlayer(playerid)
{
	if(PInfo[playerid][Sprinting] == 0) return PInfo[playerid][SprintTimer] = 0;
	else
	{
	    SetTimerEx("SprintPlayer",1,false,"i",playerid);
		if(Team[playerid] == ZOMBIE)
		{
			if(PInfo[playerid][Sprinting] == 1)
			{
				new Float:v_x,Float:v_y,Float:v_z,
			    	Float:x,Float:y,Float:z;
				GetPlayerCameraFrontVector(playerid,v_x,v_y,v_z);
 				GetPlayerCameraPos(playerid,x,y,z);
				TurnPlayerFaceToPos(playerid,v_x*2000.0+x,v_y*2000.0+y);
		    }
		}
	}
	return 1;
