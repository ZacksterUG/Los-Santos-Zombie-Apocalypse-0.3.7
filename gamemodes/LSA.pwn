...
public OnGameModeInit()
{
	...
	for(new i; i < MAX_PLAYERS;i++)
	{
		...
		XPStats[i] = TextDrawCreate(277.000000, 400.000000, "XP: 500/600");
		...
		FuelTD[i] = TextDrawCreate(221.000000, 300.000000, "Fuel: ~r~~h~ll~y~llllll~g~~h~ll");
		TextDrawBackgroundColor(FuelTD[i], -1499158273);
		TextDrawFont(FuelTD[i], 1);
		TextDrawLetterSize(FuelTD[i], 0.800000, 2.000000);
		TextDrawColor(FuelTD[i], 255);
		TextDrawSetOutline(FuelTD[i], 1);
		TextDrawSetProportional(FuelTD[i], 1);

		OilTD[i] = TextDrawCreate(327.000000, 300.000000, "Oil: ~r~ll~y~llllll~g~~h~ll");
		TextDrawBackgroundColor(OilTD[i], -1499158273);
		TextDrawFont(OilTD[i], 1);
		TextDrawLetterSize(OilTD[i], 0.800000, 2.00000);
	...
	CPSCleared = TextDrawCreate(460.000000, 97.000000, "Î÷èùåíî êîíòðîëüíûõ òî÷åê: ~r~~h~0/8");
	...
	Infection = TextDrawCreate(460.000000, 115.000000, "Çàðàæåíèå: ~r~~h~0%");
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if(PRESSED(KEY_NO) || HOLDING (KEY_NO))
    {
        if(Team[playerid] == ZOMBIE) return 0;
